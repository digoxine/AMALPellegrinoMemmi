...
class avg_pool(nn.Module):
    ...
    def __init__(self, embedding_size, out_dim, embeddings, hidden_layers=(100,100)):
        ...
        self.query = nn.Linear(embedding_size, embedding_size) #nn.Parameter(torch.randn(embedding_size, embedding_size))
    ...
    def forward(self, x):
        ...
        #q = torch.mm(x.mean(dim=1), self.query).unsqueeze(1)
        q = self.query(x.mean(dim=1)).unsqueeze(1)

        a = F.softmax(torch.sum(x*q, dim=2), dim=1)

        y = torch.sum(a.unsqueeze(2)*self.value(x), dim=1)
